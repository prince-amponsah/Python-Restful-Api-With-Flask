...
app.config['JWT_SECRET_KEY'] = 'super-secret' #Change This Url
...
def db_seed():
    ...
    test_user = User(first_name = 'Jason',
                     last_name = 'Herskey',
                     email = 'kwame@accra.com',
                     password = 'P@ssw0rd')
...
def super_simple():
    return jsonify(message='<h1>Hello from the planetory api</h1> Boo Yah')
...
def parameters():
    ...
    if age < 18:
        return jsonify(message=f"Sorry {name}, You're Not Old Enough!"), 400
    else:
        return jsonify(message=f"Sure {name}, You Qualify For This!"), 200
...
def login():
    ...
    if test:
        ...
        return jsonify(message='Login Succeded!', access_token=access_token), 201
    else:
...
def retrieve_password(email: str):
    ...
    if user:
        msg = Message("Your planetory password is " + user.password,
                      sender="admin@planetoryapigh.com",
                      recipients=[email])
        mail.send(msg)
...
def add_planet():
    ...
    if test:
        return jsonify("There's a planet already with that name"), 409
    else:
...
#This is section is for Database Models:
class User(db.Model):
    __tablename__ = 'users'
